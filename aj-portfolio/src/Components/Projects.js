import React, { Component } from "react";
import {
  Tabs,
  Tab,
  Grid,
  Cell,
  Card,
  CardActions,
  CardTitle,
  CardText,
  Button,
  CardMenu,
  IconButton,
} from "react-mdl";

class Projects extends Component {
  constructor(props) {
    super(props);
    this.state = { activeTab: 0 };
  }

  toggleCategories() {
    if (this.state.activeTab === 0) {
      return (
        <div className="project_grid">
          {/*project 1 */}
          <Card shadow={5} style={{ minWidth: "450", margin: "auto" }}>
            <CardTitle
              style={{
                style: "#fff4f2",
                height: "176px",
                background:
                  "url(data:image/png;base64,) center / cover",
              }}
            >
              Bouy
            </CardTitle>
            <CardText>
              Bouy is a suicide prevention app, aimed at giving actual resources
              to users in real time, you can get professional resources
              immediately off the home page, or get selected refined resources,
              by Civilian, Veteran, and First Responder, if that's going to take
              too long, jump right in and talk to one of our peer counselors,
              you may also purchase T-shirts which will have a a potion of the
              proceeds donated to charity to further the fight against mental
              health conditions.
              <br/><br/>React | Material UI | Firebase | React currency formater | React context API | Stripe.JS 
            </CardText>
            <CardActions border>
              {" "}
              <Button
                colored
                href="https://github.com/AjaxCodes/bouy"
                rel="noopener noreferrer"
                target="blank"
              >
                {" "}
                GitHub
              </Button>
            </CardActions>
            <CardMenu style={{ color: "#fff" }}>
              {" "}
              <IconButton name="share"></IconButton>
            </CardMenu>
          </Card>
          {/*project 2 */}
          <Card shadow={5} style={{ minWidth: "450", margin: "auto" }}>
            <CardTitle
              style={{
                style: "#fff4f2",
                height: "176px",
                background:
                  "url(data:image/png;base64,) center / cover",
              }}
            >
              youtube clone
            </CardTitle>
            <CardText>
              A Youtube clone designed to learn principals of react and using
              the Youtube API. <br/>
              <br/>React | Material UI | YouTube API
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
            </CardText>
            <CardActions border>
              {" "}
              <Button
                colored
                href="https://github.com/AjaxCodes/youtubeProject"
                rel="noopener noreferrer"
                target="blank"
              >
                {" "}
                GitHub
              </Button>
            </CardActions>
            <CardMenu style={{ color: "#fff" }}>
              {" "}
              <IconButton name="share"></IconButton>
            </CardMenu>
          </Card>
          {/*project 3 */}
          <Card shadow={5} style={{ minWidth: "450", margin: "auto" }}>
            <CardTitle
              style={{
                style: "#fff4f2",
                height: "176px",
                background:
                  "url(data:image/png;base64,) center / cover",
              }}
            >
              Covid Tracker
            </CardTitle>
            <CardText>
              App designed to show covid cases around the world, you can find
              cases by region. <br/> <br/>
              React | Material UI | Disease.Sh | Chart.JS |
              Numeral.JS | React-leaflet
              
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
              <br/>
            </CardText>
            <CardActions border>
              {" "}
              <Button
                colored
                href="https://github.com/AjaxCodes/trackerCovid"
                rel="noopener noreferrer"
                target="blank"
              >
                {" "}
                GitHub
              </Button>
              <Button
                colored
                href="https://vimeo.com/518215912?utm_content=155906697&utm_medium=social&utm_source=linkedin&hss_channel=lcp-10072839"
                rel="noopener noreferrer"
                target="blank"
              >
                {" "}
                Live Demo
              </Button>{" "}
            </CardActions>
            <CardMenu style={{ color: "#fff" }}>
              {" "}
              <IconButton name="share"></IconButton>
            </CardMenu>
          </Card>
          {/* project 4   last one */}
         
          
        </div>
       
      );
    } else if (this.state.activeTab === 1) {
      return (
        <div>
          <h1>Projects Coming Soon </h1>
          {/* project 1   */}
          {/* project 2   */}
          {/* project 3   */}
          {/* project 4   last one */}
        </div>
      );
    } else if (this.state.activeTab === 2) {
      return (
        <div>
          <Card shadow={5} style={{ minWidth: "450", margin: "auto" }}>
            <CardTitle
              style={{
                style: "#fff4f2",
                height: "176px",
                background:
                  "url(data:image/png;base64,) center / cover",
              }}
            >
              Kodning (group project)
            </CardTitle>
            <CardText>A social Media site for programers</CardText>
            <CardActions border>
              {" "}
              <Button
                colored
                href="https://github.com/c43721/Kodning"
                rel="noopener noreferrer"
                target="blank"
              >
                {" "}
                GitHub
              </Button>
            </CardActions>
            <CardMenu style={{ color: "#fff" }}>
              {" "}
              <IconButton name="share"></IconButton>
            </CardMenu>
          </Card>
          
          {/* project 2   */}
          {/* project 3   */}
          {/* project 4   last one */}
        </div>
      );
    }
  }

  render() {
    return (
      <div>
        <Tabs
          activeTab={this.state.activeTab}
          onChange={(tabId) => this.setState({ activeTab: tabId })}
          ripple
        >
          <Tab>React.JS</Tab>
          <Tab>Vue.JS</Tab>
          <Tab>Mongo DB</Tab>
        </Tabs>

        <Grid>
          <Cell col={12}>
            <div className="content">{this.toggleCategories()}</div>
          </Cell>
        </Grid>
      </div>
    );
  }
}
export default Projects;
